...
float illumination(vec3 world_position, vec3 world_normal) {
  // translate world space position into the light's view space
  ...
  if (length(light_normal) <=1.0) {
    // calculating the specularity falloff in world space instead of
    // the light's view space like the shadows are
...
void main(void) {
  if (shader_pass == 0) {
    ...
    if (space.w == -1.0) {
      ...
      float light = illumination(space.xyz, normal);
